...
MatXd AXY(Ny, Nx), DSY(Ny, Nx), YNY(Ny, Nx), SigmaY(Ny, Nx), SII_fault(Ny, Nx), SIIB(Ny, Nx);
...
int main() {
    ...
    if (timestep > 0) {
        ...
        string matrix_names[30] = {"SIIB", "OM0", "OM", "OM5", "ARSF", "BRSF", "LRSF", "RHO", "ETA0", "ETA1", "ETA5", "ETA00", "IETAPLB", "SXY0", "YNY0", "YNY00", "KKK",
                                   "GGG", "COHC", "COHT", "FRIC", "FRIT", "DILC", "TTT", "EIIB", "STRPLB", "VSLIPB", "DSY", "YNY", "ETA"}; // {"names"} has to be the same as in *matrix
        int j = 0;
        for (auto i : {&SIIB, &OM0, &OM, &OM5, &ARSF, &BRSF, &LRSF, &RHO, &ETA0, &ETA1, &ETA5, &ETA00, &IETAPLB, &SXY0, &YNY0, &YNY00, &KKK, &GGG, &COHC, &COHT, &FRIC, &FRIT,
                       &DILC, &TTT, &EIIB, &STRPLB, &VSLIPB, &DSY, &YNY, &ETA}) { // {names} *matrix
            *i = read_matrix(filename, group_matrix, matrix_names[j], dims1);
            j++;
        }

        hsize_t dims2[2] = {Ny1, Nx1};
        string matrix_names_plus[35] = {"pt", "vxs", "vys", "pf", "vxD", "vyD", "DVX0", "DVY0", "ETAB", "ETAB0", "ETAP", "ETAP0", "POR", "GGGP", "GGGB", "PTF0", "PT0", "PF0", "pt_ave",
                                        "pf_ave", "SXX0", "SYY0", "DILP", "RHOX", "RHOFX", "ETADX", "PORX", "VX0", "VXF0", "RHOY", "RHOFY", "ETADY", "PORY", "VY0", "VYF0"}; // {"names"} has to be the same as in *matrix_plus
        j = 0;
        for (auto i : {&pt, &vxs, &vys, &pf, &vxD, &vyD, &DVX0, &DVY0, &ETAB, &ETAB0, &ETAP, &ETAP0, &POR, &GGGP, &GGGB, &PTF0, &PT0, &PF0, &pt_ave, &pf_ave, &SXX0, &SYY0,
                       &DILP, &RHOX, &RHOFX, &ETADX, &PORX, &VX0, &VXF0, &RHOY, &RHOFY, &ETADY, &PORY, &VY0, &VYF0}) { // {names} *matrix_plus
        ...
        string vector_names_marker[21] = {"t_marker", "rhom", "etasm", "cohescm", "cohestm", "frictcm", "dilatcm", "fricttm", "kkkm", "xm", "ym", "etam", "etabm",
                                          "porm", "sxxm", "syym", "sxym", "vx0m", "vy0m", "ptfm", "amursfm"}; // {"names"} has to be the same as in *vec2
        j = 0;
        for (auto i : {&t_marker, &rhom, &etasm, &cohescm, &cohestm, &frictcm, &dilatcm, &fricttm, &kkkm, &xm, &ym, &etam, &etabm, &porm, &sxxm, &syym, &sxym, &vx0m,
    ...
    for (; timestep <= num_timesteps; timestep++) {
        ...
        for (int i = 1; i < Ny; i++) {
            for (int j = 1; j < Nx; j++) {
                ...
                Matrix2d ETA_temp = 2 * ETA.block(i - 1, j - 1, 2, 2);
        ...
        if (timestep % savestep == 0) {
            ...
            string matrix_names[30] = {"SIIB", "OM0", "OM", "OM5", "ARSF", "BRSF", "LRSF", "RHO", "ETA0", "ETA1", "ETA5", "ETA00", "IETAPLB", "SXY0", "YNY0", "YNY00", "KKK",
                                       "GGG", "COHC", "COHT", "FRIC", "FRIT", "DILC", "TTT", "EIIB", "STRPLB", "VSLIPB", "DSY", "YNY", "ETA"}; // {"names"} has to be the same as in *matrix
            int j = 0;
            for (auto i : {SIIB, OM0, OM, OM5, ARSF, BRSF, LRSF, RHO, ETA0, ETA1, ETA5, ETA00, IETAPLB, SXY0, YNY0, YNY00, KKK, GGG, COHC, COHT, FRIC, FRIT, DILC, TTT, EIIB, STRPLB,
                           VSLIPB, DSY, YNY, ETA}) { // {names} *matrix
            ...
            string matrix_names_plus[35] = {"pt", "vxs", "vys", "pf", "vxD", "vyD", "DVX0", "DVY0", "ETAB", "ETAB0", "ETAP", "ETAP0", "POR", "GGGP", "GGGB", "PTF0", "PT0", "PF0", "pt_ave",
                                            "pf_ave", "SXX0", "SYY0", "DILP", "RHOX", "RHOFX", "ETADX", "PORX", "VX0", "VXF0", "RHOY", "RHOFY", "ETADY", "PORY", "VY0", "VYF0"}; // {"names"} has to be the same as in *matrix_plus
            j = 0;
            for (auto i : {pt, vxs, vys, pf, vxD, vyD, DVX0, DVY0, ETAB, ETAB0, ETAP, ETAP0, POR, GGGP, GGGB, PTF0, PT0, PF0, pt_ave, pf_ave, SXX0, SYY0, DILP, RHOX, RHOFX, ETADX,
            ...
            string vector_names_marker[21] = {"t_marker", "rhom", "etasm", "cohescm", "cohestm", "frictcm", "dilatcm", "fricttm", "kkkm", "xm", "ym", "etam", "etabm",
                                              "porm", "sxxm", "syym", "sxym", "vx0m", "vy0m", "ptfm", "amursfm"}; // {"names"} has to be the same as in *vec2
            j = 0;
            for (auto i : {t_marker, rhom, etasm, cohescm, cohestm, frictcm, dilatcm, fricttm, kkkm, xm, ym, etam, etabm, porm, sxxm, syym, sxym, vx0m, vy0m, ptfm, amursfm}) { // {names} *vec2
